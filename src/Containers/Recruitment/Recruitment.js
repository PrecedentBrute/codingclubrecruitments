...
const Recruitment = (props) => {
  ...
  let buttonClass =
    "p-10 mt-10 py-2 rounded-full bg-red-500 text-gray-100 focus:outline-none hover:bg-red-600";
  // if (!props.loggedIn) {
  ...
  return (
    <div>
      <div className="recruitmentDiv rounded-lg">
        {show ? (
          <Exam person={props.person} />
        ) : (
          <div className="flex flex-col items-center">
            <Fade top>
              <div className=" flex justify-center items-center">
                <div className=" h-64 w-56 mt-24 flex flex-col justify-around items-center recruitment-card">
                  <div className="bg-red-600 h-24 w-24 rounded-full flex items-center justify-center recruitment-card-image"></div>
                  <h3 className="text-white text-center p-2">
                    {props.loggedIn && props.person !== null ? (
                      <>Welcome {props.person.name}</>
                    ) : (
